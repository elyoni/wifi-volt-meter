...
void SmartWifi::accessPointAndPasswordForm(void){
    ...
    snprintf(site,430,
            ...
            Access Point:<input type=\"text\" name=\"accessPoint\">\
            <br>\
...
class I2CChip{
    private:
        ...
        void loadCredentiasFromEEPROM(void);
    public:
        I2CChip(char chipAddress,byte numberOfInputs,ESP8266WebServer& webServer);//Use the default pins
        I2CChip(char chipAddress,byte numberOfInputs,int SDA, int SCL, ESP8266WebServer& webServer);
        void configureInputsViWebsite();
};

I2CChip::I2CChip(char chipAddress,byte numberOfInputs, ESP8266WebServer& webServer) : _chipAddress(chipAddress), _numberOfInputs(numberOfInputs) ,_webServer(webServer){
    _SDA = 4;
    _SCL = 5;
}
I2CChip::I2CChip(char chipAddress,byte numberOfInputs,int SDA, int SCL, ESP8266WebServer& webServer) : _chipAddress(chipAddress), _SDA(SDA) , _SCL(SCL), _numberOfInputs(numberOfInputs), _webServer(webServer){
}
...
void I2CChip::clearCredentiasFromEEPROM(void){
    /*
    String empty = "0000"; //Enter junk data
    empty.toCharArray(_ssid,SSID_SIZE);
    empty.toCharArray(_password,PASSWORD_SIZE);
...
void I2CChip::loadCredentiasFromEEPROM(void){
    /*
    char ok[3];
    EEPROM.begin(_eepromSize);
    EEPROM.get(_startAddress, _ssid);
...
void I2CChip::configureInputsViWebsite(){
    //
    _webServer.on("/ChipSetup",[this](){chipAddressConfigure();}); //Cofiguring the address of chip
    _webServer.on("/InputsSetup",[this](){inputAddressConfigure();}); //Cofiguring the address of chip
}

void I2CChip::chipAddressConfigure(){
    char site[260];
    snprintf(site,260,
            "<html>\
            <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">\
            <body>\
            <h1>Chip Address And Number Of Inputs Configuration</h1>\
            <form action=\"/InputsSetup\">\
            Chip I2C Address:<input type=\"text\" name=\"ChipAddress\">\
            <br>\
            Number Of Inputs:<input type=\"number\" name=\"numberOfInputs\">\
            <br>\
...
void I2CChip::inputAddressConfigure(){
    // arg(0) = "Chip I2C Address"
    // arg(1) = "Number Of Inputs"
    chipAddress = _webServer.arg(0);
    numberOfInputs = _webServer.arg(1).toInt();
    String site;
    site = "<html>\
            <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">\
            <body>\
                <h1>Inputs Address And Property Configuration</h1>\
                <form action=\"/-------------------------------------------\">"
                    for (int i = 0 ; i < numberOfInputs ; i++){
                        site += "Input" + i + " Address:<input type=\"text\" name=\"intputAddress"+ i "\">\
                                 <br>"
                    }
    site += "       <input type=\"submit\" value=\"Next\">\
                </form>\
             </body>\
             </html>";
    _webServer.send(200,"text/html",site);
